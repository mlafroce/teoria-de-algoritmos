...
fn test_quick_select_min(){
    let my_vec = vec![-2, 10, 1, 3, -5, 2, 4, 6];
    let statistic = statistic::quick_select::k_statistic(&my_vec, 0 as i32);
    assert_eq!(statistic, -5)
}
...
fn test_quick_select_mid(){
    let my_vec = vec![-2, 10, 1, 3, -5, 2, 4, 6];
    let statistic = statistic::quick_select::k_statistic(&my_vec, 3);
...
fn test_quick_select_max(){
    let my_vec = vec![-2, 10, 1, 3, -5, 2, 4, 6];
    let statistic = statistic::quick_select::k_statistic(&my_vec, (my_vec.len() - 1) as i32);
    assert_eq!(statistic, 10)
}